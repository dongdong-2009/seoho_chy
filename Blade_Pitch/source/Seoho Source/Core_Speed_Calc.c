...
/*
	...
	if (abs_position > 4095) abs_position -= 4096;
*/
	...
	if (Number_of_turns % 2 == 0)
	{
		abs_position = 	8192 - Steps_per_turn;
	}
	else abs_position = Steps_per_turn;
	if (abs_position > 8191) abs_position = 0;
	
	// HHH ¼öÁ¤ ÇÊ¿äÇÔ
	abs_position = 8192 - abs_position;

    etest1 = (double)abs_position;		
//	abs_position_final= abs_position;
...
*/
	...
	if(step_delta>(4096))		step_delta = step_delta - (8192);
	else if(step_delta<-(4096))	step_delta = step_delta + (8192);
	else				step_delta = step_delta;


	abs_step_delta = fabs(step_delta);
	...
	if (abs_step_delta > 100)
	{
		abs_position = pre_abs_position;
	}
...
//	X_delta_buf = ( (turn_delta * 4096.) +  (step_delta / 4096.));
   	...
   	X_delta_buf = step_delta / 8192.;
	X_delta = 2. * PI * X_delta_buf;
	Wrn1 = X_delta * 1250.;
	Wrpm_det1 = 60. *  X_delta_buf * 1250.;
	step_old = step_new;
	test3 = Steps_per_turn_buf;
	test8 = Steps_per_turn;
